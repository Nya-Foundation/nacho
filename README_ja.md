...
データディレクトリごとに実行するサーバープロセスは**1 つだけ**にしてください。
アプリの状態、リビジョンカウンター、WebSocket 購読はプロセスのメモリ上に
あるため、複数ワーカー（例: `uvicorn --workers 2`）で動かしたり、2 つの
サーバープロセスに同じ `--data-dir` を指定したりすると、リビジョンの不整合や
ライブ更新の取りこぼしが発生します。
...
自身のアプリケーションが設定管理をすでに許可したユーザーにのみ、キーを
書き込んでください。キーは完全なアクセス権を与えるため、一部のユーザーには
閲覧のみを許したいホストアプリケーションは、その制限を自前で実装する必要が
あります — キーを書き込まないか、信頼するユーザーにだけ UI をマウントします。
...
キーは 1 つだけで、アクセス権は全か無かです — ロールも 2 つ目のキーも
ありません。読み取り専用が必要な場合は、独立した 2 つの手段で対応します:

- 書き込むべきでない**クライアント**は `read_only=True` を指定して
  インスタンスを構築します。以降、変更操作はラウンドトリップなしに
  ローカルで `PermissionError` を送出します。
- すべての書き込みを拒否すべき**サーバー**は `--read-only` で起動します。
  呼び出し元を問わず、あらゆる変更に `403 Forbidden` を返します。
...
現在のリビジョンを `X-Nacho-Revision` で返します。`X-Nacho-Generation` は
現在のサーバープロセスを識別し、`ETag` はこの 2 つの値を組み合わせます。
書き込みには、クライアントが最後に確認したリビジョンを示す
...
server generation、アプリ名、リビジョン、設定全体、スキーマ、現在のメタデータを
含みます。generation により、一時サーバーの再起動でリビジョンがリセットされても
安全に復旧できます。書き込みは常に REST を経由する
...
ローカル書き込みはファイルに対する read-modify-write で、プロセス間ロックは
ありません。ファイル自体はアトミックに書き込まれますが、同じファイルに対して
2 つの `nacho set` が競合すると片方の更新が失われることがあります。並行して
書き込む場合はサーバー（`--remote`）に向けてください — リビジョンチェック
付きの書き込みはそのためにあります。
...
Compose はデフォルトでループバックのみにバインドし、`nacho-data` ボリュームへ
状態を永続化します。外部へ公開する前に `--api-key` を設定してください。
...
- サーバーの状態はファイルベースかつシングルプロセスです。リビジョン
  ...
  実装するための境界になります。
- 新規作成するサーバー状態・履歴ディレクトリのモードは `0700`、その JSON
  ファイルは `0600` です。既存のパーミッションは保持されます。
